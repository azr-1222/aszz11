...
bot = telebot.TeleBot('7881784396:AAGJ2k-VWz7dojT7fp5zaLqzMu4RciJItuE')
    ...
    if user_id in admin_id:
        ...
        if len(command) > 1:
            ...
            if user_to_add not in allowed_user_ids:
                ...
                response = f"User {user_to_add} Added Successfully ð."
            else:
                response = "User already exists ð¤¦ââï¸."
        else:
            response = "Please specify a user ID to add ð."
    else:
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            response = "â Usage :- /bgmi <target> <port> <time>"  # Updated command syntax
    else:
...
ð¥ /bgmi : Method For Bgmi Servers. 
...
def welcome_start(message):
    ...
    response = f'''ðð»Welcome to Your Home, {user_name}! Feel Free to Explore.
